...
//***************************************************************************************************************************************************************
	...
	UIDivWindow.prototype.init = function() {
		...
		var stockItemComponent = new ListComponent('mydiv', columns, utilUrl,
				option);
	...
	function GetMaxNo() {
		$.ajax({
			type : "post",
			url : "${ctx}/sales/erpSales/GetMaxNo",
			dataType : 'json',
				...
				.datagrid(
						{
							...
							url : "${ctx}/sales/erpSales/GetSalesDetailList?id="+ id,
							//idField: 'itemid',
							textField : 'itemDesc',
	...
	function remove() {
		...
		if ((detailid == undefined) || (detailid == "")) { //新增行的删除
			...
			//按钮
				...
				$.ajax({
					type : "post",
					url : "${ctx}/sales/erpSales/DeleteSalesDetailByid",
					dataType : 'text',
					data : {
						detailid : detailid
	...
	function SaveData() {
		if (endEditing()) {
			...
			params.id = id;
			params.no = no;                                   //销售单号
			params.depid = $.trim($("#depid").val());         //销售部门
			params.buyername = $.trim($("#buyername").val()); //买方
			params.erpSalesDetailList = content;              //明细
			...
			$.ajax({
				type : "post",
				url : "${ctx}/sales/erpSales/SaveErpSalesDetail",
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a
			href="${ctx}/sales/erpSales/erpSalesForm?id=${ErpSales.id}">销售<shiro:hasPermission
					name="sales:erpSales:view">${not empty ErpSales.id?'修改':'添加'}</shiro:hasPermission>
		</a></li>
	</ul>
	<br />
    <form:form id="inputForm" modelAttribute="erpSales"
		action="${ctx}/sales/erpSales/save" method="post" class="form-horizontal">
		<form:hidden path="id" />
		<sys:message content="${message}" />
		<div class="control-group">
			<label class="control-label">销售单号：</label>
			<div class="controls">
				<form:input path="no" id="no" htmlEscape="false" maxlength="50"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="depid" class="input-medium"
					...
					<form:option value="">请选择销售部门...</form:option>
		...
		<div class="control-group">
			...
			<div class="controls">
				 <form:input path="buyername" id="buyername" htmlEscape="false"
					rows="2" maxlength="255" class="input-xxlarge " />
			</div>
		</div>
		<div class="control-group">
