...
jQuery(function($){
	...
	var
		...
		showQuickPop = function(type){
			...
			doc.unbind('click.sidebar-box').one('click.sidebar-box', hideQuickPop);

			quickPop[0].className = 'sidebar-box-pop quick_' + type;
			popDisplayed = true;
			prevPopType = type;
			quickPop.show();
			quickPop.animate({left:-320,queue:true});
		};
	quickShell.bind('click.sidebar-box', function(e){
		e.stopPropagation();
	});
	quickPop.delegate('a.bar-close','click',function(){
		quickPop.hide();
	...
	function checkResize(){
		quickShell[view.width() > 1340 ? 'removeClass' : 'addClass']('sidebar-box-dockright');
	}
	...
	function showReturnTop(){
		quickPanel.addClass('li-gotop');
	}
	function hideReturnTop(){
		quickPanel.removeClass('li-gotop');
	}
	view.bind('scroll.go_top', resizeHandler).bind('resize.sidebar-box', scrollHandler);
	quickLinkCollapsed && quickShell.addClass('sidebar-box-min');
	resizeHandler();
	scrollHandler();
});
...
//IE8飞入购物车动画
if($('.sidebar-box li').hasClass('li-shopcart') && navigator.appName == "Microsoft Internet Explorer" && (navigator.appVersion .split(";")[1].replace(/[ ]/g,"")=="MSIE6.0" || navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion .split(";")[1].replace(/[ ]/g,"")=="MSIE7.0" || navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion .split(";")[1].replace(/[ ]/g,"")=="MSIE8.0")) {
	var lastTime = 0;
